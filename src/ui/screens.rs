...
pub fn render_syncing_screen(f: &mut Frame, sync_status: &str) {
    ...
    // Determine if we're offline or syncing
    ...
    // Split inner area for message and gauge
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
    ...
    // Render gauge (only if not offline)
    if !is_offline {
        ...
        // Show a message about when sync will occur
